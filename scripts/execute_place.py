...
def main():
    parser = argparse.ArgumentParser("Debug Pick and Place Combo")
    parser.add_argument("--env", default="FrankaShelfPNPDense-v0")
    parser.add_argument("--render", action="store_true")
    parser.add_argument("--no-render", dest="render", action="store_false")
    parser.add_argument("--max-tick", type=int, default=2000)
    ...
    for t in range(args.max_tick):
        ...
        if root.status == py_trees.common.Status.SUCCESS:
            print(f"[✓] Pick and Place SUCCESS after {t+1} ticks")
            break
        ...
        print("[✗] Pick and Place did not succeed within limit")
